...
function DocumentItem({ doc, type }) {
  const handleOpen = () => {
    if (doc.url) {
      ...
      const endpoint = type === 'voucher' ? `/bookings/${doc.bookingId}/voucher` : `/payments/${doc.paymentId}/invoice`;
    ...
    <div className="flex items-center gap-4 p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <div className="font-semibold text-gray-800 dark:text-gray-200 text-sm truncate">
          {doc.title || (type === 'voucher' ? `Voucher κράτησης ${doc.bookingNumber || ''}` : `Τιμολόγιο ${doc.invoiceNumber || ''}`)}
        </div>
        {doc.vehicle && (
      ...
      <button
        onClick={handleOpen}
        className="flex items-center gap-1.5 px-3 py-1.5 bg-brand-teal text-white text-xs font-bold rounded-lg hover:bg-teal-700 transition-colors flex-shrink-0"
      >
        <span>📥</span>
        <span>Λήψη</span>
      </button>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        ...
        {vouchers.length === 0 ? (
          <EmptyState icon="🎫" title="Δεν υπάρχουν vouchers" />
        ) : (
          <div className="divide-y divide-gray-100 dark:divide-gray-700">
            {vouchers.map(v => <DocumentItem key={v._id} doc={v} type="voucher" />)}
          </div>
        )}
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        ...
        {invoices.length === 0 ? (
          <EmptyState icon="🧾" title="Δεν υπάρχουν τιμολόγια" />
        ) : (
          <div className="divide-y divide-gray-100 dark:divide-gray-700">
            {invoices.map(inv => <DocumentItem key={inv._id} doc={inv} type="invoice" />)}
          </div>
        )}
      </div>
    </div>
